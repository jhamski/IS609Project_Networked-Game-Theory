...
one.degree: finds the 'Footsoldiers' - nodes with degree of one. If there are no one-degree nodes, 40% of the network is randomly assigned as a starting point. 
```{r}
one.degree <- function(g){
  ...
  if(length(degree.list) == 0){
    num.samples <- round(length(V(g))*0.4, 0)
